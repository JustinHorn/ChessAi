...
public class GetSpecialMoves_FromBoard {
	...
	private static boolean isTurmAlright(Board b, Figure me, List<Figure> figures, boolean leftOrRight) {
		...
		return turm instanceof Turm && turm.isWhite() == me.isWhite() && !b.hasFigure_beenMoved(turm.getId()) && figures.contains(turm);
	}
	...
	private static boolean are_fields_unthreadend(Board b, List<Figure> horizontalLineOfFigures, boolean isWhite) {
		List<Position> pos = horizontalLineOfFigures.stream().map(f -> b.getPosition_of_FigureWithId(f.getId()))
			...
			if (BoardUtils.threats_at_Position(b, pos.get(0), isWhite).size() != 0) {
				return false;
			}
			
			if (BoardUtils.threats_at_Position(b, pos.get(1), isWhite).size() != 0) {
				return false;
			} 
